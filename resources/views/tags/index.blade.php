                            ...
                            <form action="{{ route('tags.index') }}" method="GET">
                                <div class="input-group">
                                    <input name="keyword" value="{{ request()->get('keyword') }}" type="search"
                                        class="form-control"
                                        placeholder="{{ trans('tag.form_control.input.search.placeholder') }}">
                                    ...
                                    <div>
                                        ...
                                        @can('tag_delete')
                                            <form class="d-inline" role="alert"
                                                alert-text="{{ trans('tag.alert.delete.message.confirm', ['title' => $tag->title]) }}"
                            ...
                            <p>
                                <strong>
                                    @if (request()->get('keyword'))
                                        {{ trans('tag.label.no_data.search', ['keyword' => request()->get('keyword')]) }}
                                    @else
                                        {{ trans('tag.label.no_data.fetch') }}
                                    @endif
    ...
    <script>
        $(document).ready(function() {
            ...
            $("form[role='alert']").submit(function(event) {
                ...
                Swal.fire({
                    title: "{{ trans('tag.alert.delete.title') }}",
                    text: $(this).attr('alert-text'),
                    icon: 'warning',
                    allowOutsideClick: false,
                    showCancelButton: true,
                    cancelButtonText: "{{ trans('tag.button.cancel.value') }}",
                    reverseButtons: true,
                    confirmButtonText: "{{ trans('tag.button.delete.value') }}",
                }).then((result) => {
